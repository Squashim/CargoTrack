...
function SidebarProvider({
  ...
  const isMobile = useMediaQuery(`(width <= ${BREAKPOINTS.md})`);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        data-side={side}
        className={cn(
          'fixed inset-y-0 top-20 z-10 hidden h-[calc(100svh-5rem)] w-(--sidebar-width) transition-[left,right,width] duration-200 ease-linear data-[side=left]:left-0 data-[side=left]:group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)] data-[side=right]:right-0 data-[side=right]:group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)] md:flex',
...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  const message = state === 'collapsed' ? t('sidebar.open') : t('sidebar.close');
  ...
  const toggleButton = (
    <Button
      ...
      variant="ghost"
      size="icon-lg"
      title={!isMobile ? undefined : message}
      aria-label={message}
